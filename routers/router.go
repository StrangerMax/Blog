...
func Router() {
	...
	*/
	...
	//auth.Use(middleware.AuthMiddleware())
	{
		...
		// 评论模块
		auth.GET("comment/list", v1.GetCommentList)       // GetCommentList 后台查询评论列表
		auth.DELETE("delcomment/:id", v1.DeleteComment)   // DeleteComment 删除评论
		auth.PUT("checkcomment/:id", v1.CheckComment)     // CheckComment 通过审核
		auth.PUT("uncheckcomment/:id", v1.UncheckComment) // UncheckComment 撤下评论审核
	}
	...
	*/
	router := r.Group("api/v1")
	{
		...
		// 评论模块
		router.POST("addcomment", v1.AddComment)               // AddComment 新增评论
		router.GET("comment/info/:id", v1.GetComment)          // GetComment 获取单个评论信息
		router.GET("commentfront/:id", v1.GetCommentListFront) // GetCommentListFront 展示页面显示评论列表
		router.GET("commentcount/:id", v1.GetCommentCount)     // GetCommentCount 获取评论数量
	}
